...
JNIEXPORT void JNICALL Java_com_setl_ed25519_Ed25519DonnaJNI_testCryptoLibrary(JNIEnv *env, jobject thisObj)
{
	test_main();
	test_batch();
  ...
  (JNIEnv *env, jobject thisObj, jint size)
{
	...
	unsigned char* pkBytes = (unsigned char*) (*env)->GetByteArrayElements(env, ret, 0);
	RAND_bytes(pkBytes, size);
	(*env)->ReleaseByteArrayElements(env, ret, (jbyte*)pkBytes, 0);
	return ret;
}
...
JNIEXPORT jbyteArray JNICALL Java_com_setl_ed25519_Ed25519DonnaJNI_createPublicKey
  (JNIEnv *env, jobject thisObj, jbyteArray private_key) {

	jbyteArray public_key = (*env)->NewByteArray(env, sizeof(ed25519_public_key));

	unsigned char* publicKeyBytes = (unsigned char*) (*env)->GetByteArrayElements(env,  public_key, 0);
	unsigned char* privateKeyBytes = (unsigned char*) (*env)->GetByteArrayElements(env, private_key, 0);

	ed25519_publickey(privateKeyBytes, publicKeyBytes);

	(*env)->ReleaseByteArrayElements(env, public_key, (jbyte*)publicKeyBytes, 0);
	(*env)->ReleaseByteArrayElements(env, private_key, (jbyte*)privateKeyBytes, 0);

	return public_key;
}

JNIEXPORT jbyteArray JNICALL Java_com_setl_ed25519_Ed25519DonnaJNI_createSignature
  (JNIEnv *env, jobject thisObj, jbyteArray message, jbyteArray privateKey, jbyteArray publicKey)
{

  unsigned char* messageBytes    = (unsigned char*) (*env)->GetByteArrayElements(env, message, 0);
  unsigned char* privateKeyBytes = (unsigned char*) (*env)->GetByteArrayElements(env, privateKey, 0);
  unsigned char* publicKeyBytes  = (unsigned char*) (*env)->GetByteArrayElements(env, publicKey, 0);

  jbyteArray sig=(*env)->NewByteArray(env, sizeof(ed25519_signature));
  unsigned char* sigBytes = (unsigned char*)    (*env)->GetByteArrayElements(env, sig, 0);

  ed25519_sign(messageBytes, (*env)->GetArrayLength(env, message), privateKeyBytes, publicKeyBytes, sigBytes);
  ...
  (*env)->ReleaseByteArrayElements(env, sig,        (jbyte*)sigBytes,        0);
  return sig;
}

JNIEXPORT jboolean JNICALL Java_com_setl_ed25519_Ed25519DonnaJNI_verifySignature(
  JNIEnv *env, jobject thisObj, jbyteArray message, jbyteArray publicKey, jbyteArray sig) {

  unsigned char* messageBytes    = (unsigned char*) (*env)->GetByteArrayElements(env, message, 0);
  unsigned char* publicKeyBytes  = (unsigned char*) (*env)->GetByteArrayElements(env, publicKey, 0);
  unsigned char* sigBytes = (unsigned char*)    (*env)->GetByteArrayElements(env, sig, 0);
  int msgLength=(*env)->GetArrayLength(env, message);

  jboolean ret=(ed25519_sign_open(messageBytes, msgLength, publicKeyBytes, sigBytes)==0);
